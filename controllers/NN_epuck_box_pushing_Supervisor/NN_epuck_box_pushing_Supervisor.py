...
def evaluate(individual):
    ...
    fileGreen = "..\\NN_epuck_box_pushing_Green\cNN.csv"
    if os.path.exists(fileGreen):
        os.remove(fileGreen)
            
    with open("..\\NN_epuck_box_pushing_Green\cNN.csv", "w") as fileGreen:
        writer = csv.writer(fileGreen)
        writer.writerow(individual)
    
    
    
    fileRed = "..\\NN_epuck_box_pushing_Red\cNN.csv"
    if os.path.exists(fileRed):
        os.remove(fileRed)
            
    with open("..\\NN_epuck_box_pushing_Red\cNN.csv", "w") as fileRed:
        writer = csv.writer(fileRed)
        writer.writerow(individual)
        
        
        
    fileBlue = "..\\NN_epuck_box_pushing_Blue\cNN.csv"
    if os.path.exists(fileBlue):
        os.remove(fileBlue)
            
    with open("..\\NN_epuck_box_pushing_Blue\cNN.csv", "w") as fileBlue:
    ...
    while reset == 0:
        ...
        file = "..\\NN_epuck_box_pushing_Red\FoundPixelsRed.txt"
        if os.path.exists(file):
            #print(1)
            OtherFile = open(file, "r")
            for element in OtherFile.read():
        ...
        file = "..\\NN_epuck_box_pushing_Green\FoundPixelsGreen.txt"
        if os.path.exists(file):
            #print(1)
            OtherFile = open(file, "r")
            for element in OtherFile.read():
        ...
        file = "..\\NN_epuck_box_pushing_Blue\FoundPixelsBlue.txt"
        if os.path.exists(file):
            #print(1)
            OtherFile = open(file, "r")
            for element in OtherFile.read():
        ...
        if trans_field.getSFVec3f()[0] > 1.5 or supervisor.getTime() > SimulationTimeLimit or failed == 1:
            ...
            for j in range(generationNum * populationLimit):
                filePos = "..\\NN_epuck_box_pushing_Supervisor\Positions" + str(j) + ".csv"
